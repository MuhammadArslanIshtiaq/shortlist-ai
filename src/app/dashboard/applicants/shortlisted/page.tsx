  ...
  const fetchApplicants = async () => {
    try {
      ...
      const applicantsWithJobTitles = await Promise.all(
        shortlistedApplicants.map(async (applicant: Applicant) => {
      ...
      setError('Failed to load shortlisted applicants. Please try again.');
    } finally {
      setLoading(false);
    }
  };
                ...
                {filteredApplicants.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-8 text-center text-gray-500">
                      {applicants.length === 0 ? 'No shortlisted applicants found' : 'No shortlisted applicants match your search criteria'}
                    </td>
                  </tr>
                ) : (
